...
func _build_far_silhouettes(biome: StringName, corruption: float) -> void:
	...
	for row in 2:
		...
		# Cool dark blue-grey only — never tinted toward the (often brown/tan)
		# horizon, so distant relief reads as shadowed hills, not brown blobs.
		var base_dark := Color(0.13, 0.15, 0.20)
		var silhouette_col: Color = base_dark.darkened(0.1 + float(row) * 0.2)
		silhouette_col = silhouette_col.lerp(Color(0.14, 0.04, 0.16), corruption * 0.4)
		for i in count:
	...
	var mid_col: Color = Color(0.16, 0.18, 0.23).lerp(Color(0.14, 0.04, 0.16), corruption * 0.3)
	for i in 7:
